...
public abstract class Vehicle implements VehicleObserver {
  ...
  public boolean provideInfo() {
    ...
    if (!isTripComplete()) {
      ...
      String type = "";
      if (this instanceof SmallBusDecorator) {
        type = SmallBus.SMALL_BUS_VEHICLE;
      } else if (this instanceof LargeBusDecorator) {
        type = LargeBus.LARGE_BUS_VEHICLE;
      } else if (this instanceof ElectricTrainDecorator) {
        type = ElectricTrain.ELECTRIC_TRAIN_VEHICLE;
      } else if (this instanceof DieselTrainDecorator) {
